...
class ModuleCreator():
    ...
    def create_and_publish_module(self, title, server, credentials, workgroup_url=''):
        info_str = "Creating module: "+title+" on "+server+" with user: "
        ...
        if workgroup_url is not '':
            self.logger.info("Creating module in workgroup: " + workgroup_url)
    ...
    def run_create_and_publish_module(self, title, server,credentials, logfile, workgroup_url=''):
        self.setup(logfile)
